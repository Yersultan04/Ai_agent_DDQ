...
export async function POST(req: Request, ctx: { params: Promise<{ projectId: string }> }) {
  try {
    ...
    return NextResponse.json(
      { ok: true, runId: run.id, status: run.status, progress: run.progress, startedAt: run.startedAt.toISOString() },
      { status: 202 }
    );
  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : String(error);
    return NextResponse.json({ error: "Failed to queue run", details: message }, { status: 500 });
  }
}
